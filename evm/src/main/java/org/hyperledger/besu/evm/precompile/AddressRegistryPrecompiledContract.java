...
public class AddressRegistryPrecompiledContract extends AbstractPrecompiledContract {
  ...
  private UInt256 storageSlot(final Bytes address) {
    final Bytes slotKey = Bytes.concatenate(Bytes32.wrap(REGISTRY_SLOT), Bytes32.leftPad(address));
  ...
  private Bytes contains(final MutableAccount contract, final Bytes calldata) {
    final UInt256 slot = storageSlot(calldata.slice(12, 20));
  ...
  private Bytes addToRegistry(
    ...
    if (onlyOwner(contract, senderAddress).isZero()) {
      ...
      final Bytes toBytes = calldata.slice(12, 20);
      final Address initiatorAddress = Address.wrap(calldata.slice(44, 20));
      // preventing add zero address to register
      if (Address.wrap(calldata.slice(12, 20)).equals(Address.ZERO) || initiatorAddress.equals(Address.ZERO)) {
        return FALSE;
      }
      final UInt256 slot = storageSlot(toBytes);
